...
# @register.simple_tag
# def render_delete_modal(table):
#     if hasattr(table, 'get_delete_modal_html'):
#         return table.get_delete_modal_html()
#     return ''
...
def render_delete_modal(context, table):
    if hasattr(table, 'is_deletable_table') or context.get('include_delete_modal', False):
        # Get the confirmation message from the table, or use a default
        delete_confirm_message = getattr(table, 'delete_confirm_message', "Are you sure you want to delete this record?")
